...
<template>
    <div class="wrap">
      <input type="radio" :id="radioLink + _uid" value="internalLink" v-model="buttonLinkType" @change="setLinkType">
      <label :for="radioLink + _uid">Internal link</label>
      <input type="radio" :id="radioUrl + _uid" value="url" v-model="buttonLinkType" @change="setLinkType">
      <label :for="radioUrl + _uid">Url</label>
      <br>
      <template v-if="!schema.preview">
      <div v-if="buttonLinkType === 'internalLink' ||  buttonLinkType == null">
        <input
    ...
    export default {
        ...
        data () {
            return {
                ...
                selectedPath: null,
                withLinkTab: true,
                buttonLinkType: this.model.buttonLinkType,
                radioLink: 'internalLink',
                radioUrl: 'url'
            }
        },
        created(){
            this.buttonLinkType = this.buttonLinkType || 'internalLink'
        },
        computed: {
            sanitizedValue: {
                get () {
        ...
        methods: {
            setLinkType(){
                this.model.buttonLinkType = this.buttonLinkType;
            },
            onCancel(){
                this.isOpen = false
            },
